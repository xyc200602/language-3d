...
### 6.2 根目录只允许存在
`README.md`、`AGENTS.md`、`CLAUDE.md`、`pyproject.toml`、`LICENSE`、`.env(.example)`、`.gitignore`、`项目期望.txt`、以及标准目录（`src/ tests/ docs/ scripts/ examples/ data/ logs/`）。

**任何其他文件出现在根目录 = 违规。** 发现了顺手清掉或归档。

### 6.3 已知失败清单（skip 必须登记于此）

**当前本地环境（2026-07-08 验证）**：MuJoCo 3.9.0、VTK 9.3.1、FCL 0.7.0、trimesh 4.12.2、scipy/numpy 均已安装。FreeCAD 1.1 已安装于 `C:\Users\xyc\AppData\Local\Programs\FreeCAD 1.1\bin\python.exe`，`_find_freecad_python()` 可定位。MuJoCo/VTK/FCL/FreeCAD 测试**真实运行**。

**真正剩余的已知失败/限制：**
- FreeCAD-dependent integration tests（test_e2e_freecad / test_part_validator / test_vlm_preview_stls / test_part_library_e2e / test_advanced_modeling 等）: conftest 在 `_find_freecad_python()` 返回 None 时 auto-skip。装好 FreeCAD CLI 并让 `_find_freecad_python` 能找到后即恢复运行。(since 2026-06-29)
- e2e / api 标记的测试: 需要 `GLM_API_KEY`，无 key 时 skip。
- `solve_ik` 生产路径有 2 种求解器（解析 3DOF / CCD）；实验性 `JacobianIKSolver` 类独立存在（test_dual_arm_ik.py 验证），未接入 solve_ik dispatch。(since 2026-07-07)

**历史修复记录**（2026-07-03 ~ 07-08，全部 PASS，详见 `docs/history/progress.txt`）：
test_gripper_finger_geometry / test_message_bus / test_wheel_rotation_fix / test_arm_pipeline_fix / test_sim_mujoco / test_web_complex_robot —— 详见 progress.txt。

**§3.4 回归教训**：2026-07-05 修 `test_gripper_finger_geometry` 时 commit 前没跑 4wheel_dual_arm e2e，导致手指对进入 FCL 碰撞管线产生假阳性碰撞（56.8mm gap 被报为 overlap），连续 2 次 FAIL。**改了 src/ 必须 commit 前跑 e2e。**
