...
- test_arm_pipeline_fix::test_ensure_arm_default_angles_* (4 tests): pre-existing failure on clean tree — default-angle injection assertions stale vs current `_ensure_arm_default_angles` (which now excludes wheels and groups per-arm). Confirmed failing before any recent changes. (since 2026-06-28)
- test_wheel_rotation_fix / test_gripper_finger_geometry / test_message_bus (5 tests): pre-existing failures on clean master, unrelated to recent work. (since 2026-06-28)
