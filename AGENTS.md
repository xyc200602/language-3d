...
- 改 `vlm_verify.py` 的 `_geometric_prevalidation`（adjacency / collision / finger 逻辑——2026-07-05 回归实例）
...
**当前本地环境（2026-07-03 验证）**：MuJoCo 3.9.0、VTK 9.3.1、FCL 0.7.0、trimesh 4.12.2、scipy/numpy 均已安装。FreeCAD 作为 Python 模块未装（CLI 可能在系统 PATH 外，`_find_freecad_python` 返回 None）。因此 conftest 只 auto-skip FreeCAD 依赖的 integration 测试；MuJoCo/VTK/FCL 测试**真实运行**。

**真正剩余的已知失败/限制：**
- ~~IK solver 收敛（遗留）~~：已修复。`solve_ik` 生产路径有 2 种求解器（解析 3DOF / CCD），另有实验性 JacobianIKSolver 类（独立，未接入 solve_ik dispatch）。test_ik_solver.py 31 passed，含 error_mm < 1.0 收敛断言。(since 2026-07-07)
- FreeCAD-dependent integration tests（test_e2e_freecad / test_part_validator / test_vlm_preview_stls / test_part_library_e2e / test_advanced_modeling 等）: conftest 在 `_find_freecad_python()` 返回 None 时 auto-skip。本地 FreeCAD 未配置时这些跳过是真实的，不是幻觉。装好 FreeCAD CLI 并让 `_find_freecad_python` 能找到后即恢复运行。(since 2026-06-29)
- e2e / api 标记的测试: 需要 `GLM_API_KEY`，无 key 时 skip。

**本会话修复的（2026-07-03，全部 PASS，保留记录供追溯）：**
- ~~test_gripper_finger_geometry~~: `_geometric_prevalidation` 把所有 siblings（含夹爪手指）加入 adjacency skip-set，掩盖手指重叠。排除手指对（手指是平行爪，绝不能重叠）。
- ~~test_message_bus~~: `message_bus.py:47` `NameError: logger` —— 缺 `import logging`。真实 latent bug（错误路径会崩）。
- ~~test_wheel_rotation_fix (3 tests)~~: 断言旧的 solver 后处理（R_x±90°），该后处理被有意移除（`part_feature_engine` 把 orient_axis 烤进 STL）。断言改为 identity solver rotation。
- ~~test_arm_pipeline_fix (5 tests)~~: 三个 test-staleness 根因——fixture 用旧 anchor 约定、断言旧 angle pattern（zig-zag 取代全负）、monkeypatch 打错模块（应打 vlm_verify 不是 assembly_generator）。
- ~~test_sim_mujoco::test_load_existing_4dof_arm~~: 断言 `fixed 合并: 4`，实际是 6（example URDF 从 11 links 增长到 13 links）。stale 结构计数断言。
- ~~test_web_complex_robot (8 tests)~~: God-Module 拆分遗留——`_get_default_robot_data` 误留 slicing.py + 16 个 wrong-depth relative import（`..tools` 应为 `...tools`）。这些端点从 clean checkout 起就 500。

**2026-07-05 回归教训（违反 §3.4 的实例）：**
- 修 `test_gripper_finger_geometry` 时把手指对从 `_adjacent_pairs` 排除（commit `1b8d59d`），**commit 前没跑 4wheel_dual_arm e2e**。结果手指对进入 FCL 碰撞管线，FCL 的 box-mesh 对旋转 30° 的 60mm 细长手指产生假阳性碰撞（56.8mm gap 被报为 overlap），导致 4wheel_dual_arm 连续 2 次 FAIL。修复方式：分离 `_adjacent_pairs`（同位置检查，手指包含）和 `_collision_skip_pairs`（AABB→FCL 管线，手指排除），手指重叠由 Check 5（距离检测 ≥25mm）负责。**再次印证 §3.4：改了 src/ 必须 commit 前跑 e2e，不能只跑 unit。**
