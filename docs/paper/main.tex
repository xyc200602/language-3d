...
We evaluate on six robot configurations (2--6-DOF arms and a 4-wheel dual-arm mobile robot) with automated scoring across 41--43 checks per case, achieving an average score of 94.6\% (range 92.7--95.3\%), with all cases passing MuJoCo physics stability and grasp tests.
\end{abstract}
...
\begin{table}[h]
\centering
\caption{Benchmark results across 6 robot configurations.}
\label{tab:results}
\small
...
\midrule
2dof\_arm & 95.1\% & 39/0/2 & err=0.0$^\circ$ & PASS \\
3dof\_arm & 95.1\% & 39/0/2 & err=0.0$^\circ$ & PASS \\
4dof\_arm & 95.1\% & 39/0/2 & err=0.0$^\circ$ & PASS \\
5dof\_arm & 95.1\% & 39/0/2 & err=0.0$^\circ$ & PASS \\
6dof\_arm & 92.7\% & 38/1/2 & err=0.0$^\circ$ & PASS \\
4wheel\_dual\_arm & 95.3\% & 41/0/2 & err=0.0$^\circ$ & PASS \\
\midrule
\textbf{Average} & \textbf{94.6\%} & & & \textbf{6/6} \\
\bottomrule
\end{tabular}
\end{table}

Five of six evaluated cases achieve $\geq$\,95\% with zero critical failures. The 6dof\_arm case (92.7\%) had a single VLM verification failure (proportion validation false-negative), not a geometry or physics error. All six cases pass MuJoCo physics stability (err=0.0$^\circ$) and grasp tests. Two additional cases (7dof\_arm, humanoid) are defined in the test infrastructure but were not completed within the evaluation time budget.
