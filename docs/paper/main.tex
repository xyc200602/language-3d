...
\begin{abstract}
% ===================================================================
We present \textbf{Language-3D}, a multi-agent system that converts natural language descriptions into complete, manufacturable robot assembly packages---including STL/STEP meshes, URDF models, bills of materials (BOM), firmware, and ROS2 packages. Unlike prior work that generates either single CAD parts~\cite{cadllama2025, stepllm2026}, voxel-based block assemblies~\cite{bloxnet2025}, or URDF-only morphologies~\cite{robomorph2024}, Language-3D produces end-to-end engineering artifacts that can be directly 3D-printed, assembled with commercial off-the-shelf (COTS) fasteners, and deployed in simulation.
...
\textbf{The gap}: No existing system produces a \emph{complete, manufacturable package}---the full set of artifacts (STL, STEP, URDF, BOM, firmware, assembly guide, ROS2 package) needed to actually build and deploy a robot. We define this as the \textbf{NL~$\to$~Manufacturable Robot Assembly Package} task.
...
\begin{enumerate}
    ...
    \item \textbf{System}: Language-3D, a multi-agent pipeline producing STL/STEP/URDF/BOM/firmware/ROS2 from NL, with 8 real connection types and a 56-part COTS knowledge base.
...
ArtiCAD~\cite{articad2026} is the closest prior work: a training-free multi-agent system generating editable, articulated CAD assemblies via code generation. It exports URDF files (confirmed in~\cite{articad2026}, §6) but does not produce STL/STEP meshes, BOMs, firmware, or ROS2 packages, and lacks physical simulation validation. Blox-Net~\cite{bloxnet2025} (ICRA 2025) combines VLM supervision with physics simulation for generative design-for-robot-assembly, but is limited to voxel blocks. RoboMorph~\cite{robomorph2024} evolves modular robot URDFs via LLM, producing morphology configurations without CAD geometry or assembly connections. RoboGen~\cite{robogen2024} (ICML 2024) focuses on automated skill learning via generative simulation, not robot body generation.
...
Three repeated runs of the 4dof\_arm case (identical prompt) yielded scores of 95.1\%, 95.1\%, 95.1\% (mean=95.1\%, $\sigma$=0.0\%), demonstrating full determinism for structured-JSON assembly generation.

\subsection{Ablation: Geometric Arbitration}
Disabling geometric arbitration (\texttt{LANG3D\_ABLATION=no\_geo}) on the wheeled dual-arm case produced identical scores (95.3\%, 2 runs), confirming that on clean deterministic-compose output the geometric channel does not change the final score. Its value lies in \emph{preventing VLM false-negative dead-loops}---when the VLM incorrectly rejects valid geometry, the geometric oracle overrides the rejection, avoiding wasteful regeneration cycles.
...
\begin{itemize}
    \item Manufacturing quality is not physically verified (no 3D-printed prototype yet).
    \item Firmware is template-generated (not deployed on real hardware).
    ...
    \item Benchmark is limited to two cases; broader evaluation with diverse prompts is needed.
\end{itemize}
...
\section{Conclusion}
% ===================================================================
We presented Language-3D, a system that converts natural language into complete, manufacturable robot assembly packages. By combining a COTS knowledge base, real CAD connection features, dual-channel verification, and MuJoCo physics validation, the system achieves 95\%+ quality on benchmark cases with deterministic reproducibility. Future work includes physical manufacturing verification, broader benchmark datasets, and real-time motion planning.
